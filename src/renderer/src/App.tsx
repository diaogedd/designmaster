...
function App(): React.JSX.Element {
  ...
  const isLogin = useSettingsStore((s) => s.isLogin)
  const [settingsHydrated, setSettingsHydrated] = useState(() =>
    useSettingsStore.persist.hasHydrated()
  )
  const a = useSettingsStore.getState()
  const b = useProviderStore.getState()
  console.log('useSettingsStore:', a)
  console.log('useProviderStore:', b)
  ...
  // Check if a stored token exists in the main process on startup
  useEffect(() => {
    void ipcClient
      .invoke('login:check')
      .then((res) => {
        if ((res as { hasToken: boolean }).hasToken) {
          useSettingsStore.getState().setLogin(true)
        }
      })
      .catch(() => {
